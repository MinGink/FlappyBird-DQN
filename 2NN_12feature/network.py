...
def dqn():
    ...
    with tf.name_scope('input'):
        s = tf.placeholder("float", [None, 12],name='input')
        a = tf.placeholder("float", [None, ACTIONS],name='q')
        y = tf.placeholder("float", [None],name='y')    

    # layer 1
    with tf.name_scope('Layer_1'):
        W_fc1 = tf.Variable(tf.truncated_normal([12,128], stddev=0.1)) 
...
def train(s,a,y,readout,train_step,merged,sess):
    ...
    do_nothing[0] = 1
    data,r_0,terminal,RESULT = game_state.frame_step(do_nothing)
    s_t = list(data.values())
    ...
    for t in range(TRAINRANGE):
        ...
        data,r_t,terminal,RESULT = game_state.frame_step(a_t)
        s_t1 = list(data.values())
        ...
        print("TIMESTEP", t, "/ STATE", state, \
                ...
                "/ Q_MAX %e" % np.max(readout_t),"Result is ",str(Final))
